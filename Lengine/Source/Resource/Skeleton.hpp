...
public:
    ...
    const Bone& GetBone( const size_t a_Index ) const { return const_cast< Skeleton* >( this )->GetBone( a_Index ); }
    Bone& GetBone( const size_t a_Index ) { return m_Bones[ a_Index ]; }
    const Bone& GetBone( const string& a_Name ) const { return const_cast< Skeleton* >( this )->GetBone( a_Name ); }
