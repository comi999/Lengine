...
public:
	...
	const vector< MeshVertex >& GetVertices() const { return const_cast< RMesh* >( this )->GetVertices(); }
	vector< MeshVertex >& GetVertices() { return m_Vertices; }
	const vector< MeshTriangle >& GetTriangles() const { return const_cast< RMesh* >( this )->GetTriangles(); }
	vector< MeshTriangle >& GetTriangles() { return m_Triangles; }
	const vector< vec2 >& GetUVChannel( const size_t a_Index ) const { return const_cast< RMesh* >( this )->GetUVChannel( a_Index ); }
	vector< vec2 >& GetUVChannel( const size_t a_Index );
	size_t GetUVChannelCount() const { return m_UVChannelCount; }
	const vector< MeshBoneWeight >& GetBoneWeights() const { return const_cast< RMesh* >( this )->GetBoneWeights(); }
