...
ResourcePack RResource::s_LoadedResources;
...
ResourcePack RResource::LoadFile( const string& a_Name, const string& a_Path )
{
	...
	{
		const aiMesh* Mesh = a_Scene->mMeshes[ a_Index ];
		const MeshHandle Result = MakeResource< RMesh >( a_Name, a_Path );
		...
		// Handle triangles.
		Result->m_Triangles.resize( Mesh->mNumFaces );
		if ( Mesh->HasFaces() )
		{
			for ( size_t i = 0; i < Mesh->mNumFaces; ++i )
			{
				...
				Triangle.x = Mesh->mFaces[ i ].mIndices[ 0u ];
				Triangle.y = Mesh->mFaces[ i ].mIndices[ 1u ];
				Triangle.z = Mesh->mFaces[ i ].mIndices[ 2u ];
			}
		}

		// Handle UV's.
		auto HandleUVChannel = [ &Result, Mesh ]( const size_t a_ChannelIndex )
		{
			...
			Result->m_UVChannelNames[ a_ChannelIndex ] = Mesh->mTextureCoordsNames ? Mesh->mTextureCoordsNames[ a_ChannelIndex ]->C_Str() : "";
			vector< vec2 >& UVChannel = Result->m_UVChannels[ a_ChannelIndex ];
			const aiVector3D* From = Mesh->mTextureCoords[ a_ChannelIndex ];
			UVChannel.resize( Mesh->mNumVertices );
		...
		Result->m_UVChannelCount = Mesh->GetNumUVChannels();
		...
		if ( Mesh->HasBones() )
		{
			Result->m_BoneNames.resize( Mesh->mNumBones );
			Result->m_BoneWeights.resize( Mesh->mNumVertices );

			// We need to track what nodes in the scene are bone nodes for the mesh.
			vector< const aiNode* > BoneNodes( Mesh->mNumBones );
		...
		ResultResourcePack.AddResource( Result, Mesh->mName.C_Str() );
	};
	...
	{
		const aiSkeleton* Skeleton = a_Scene->mSkeletons[ a_Index ];
		const SkeletonHandle Result = MakeResource< RSkeleton >( a_Name, a_Path );

		Result->m_Bones.resize( Skeleton->mNumBones );
		for ( size_t i = 0; i < Skeleton->mNumBones; ++i )
		{
			const aiSkeletonBone* SkeletonBone = Skeleton->mBones[ i ];
		...
		ResultResourcePack.AddResource( Result, Skeleton->mName.C_Str() );

		// Look for a mesh with same name and add this skeleton to it.
		const MeshHandle Mesh = ResultResourcePack.GetResourceByAlias< RMesh >( Skeleton->mName.C_Str() );

		// Add this skeleton as a child to the mesh, making it a skeletal mesh.
		Mesh->m_Children.AddResource( Result, "Skeleton" );
	};
	...
	{
		const aiAnimation* Animation = a_Scene->mAnimations[ a_Index ];
		const AnimationHandle Result = MakeResource< RAnimation >( a_Name, a_Path );
		Result->m_Channels.reserve( Animation->mNumChannels );
		Result->m_Duration = ( RAnimation::TimeType )( Animation->mDuration / Animation->mTicksPerSecond );
		...
		for ( size_t i = 0; i < Animation->mNumChannels; ++i )
		{
			...
			for ( size_t j = 0; j < Channel->mNumPositionKeys; ++j )
			{
				...
				NewPosition.Time = ( RAnimation::TimeType )PositionKey.mTime;
			...
			for ( size_t j = 0; j < Channel->mNumRotationKeys; ++j )
			{
				...
				NewRotation.Time = ( RAnimation::TimeType )RotationKey.mTime;
			...
			for ( size_t j = 0; j < Channel->mNumScalingKeys; ++j )
			{
				...
				NewScale.Time = ( RAnimation::TimeType )ScaleKey.mTime;
			...
			ResultResourcePack.AddResource( Result, Animation->mName.C_Str() );
		}
	};
