...
bool ImGui::DragFloatXYZ(bool IsColour, const char* label, float v[3], float v_speed, float v_min, float v_max, const char* display_format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    {
        // My STUFF DELETE!
        ImVec4 TextColour = ImVec4(255, 255, 255, 255);
        const char* Label;
        switch (i)
        {
        case 0:
            TextColour = ImVec4(255, 0, 0, 255);
            Label = ": x ##v";
            break;
        case 1:
            TextColour = ImVec4(0, 255, 0, 255);
            Label = ": y ##v";
            break;
        case 2:
            TextColour = ImVec4(0, 0, 255, 255);
            Label = ": z ##v";
            break;
        }
        PushStyleColor(0, TextColour);

        if (IsColour)
            Label = "";

        PushID(i);
        value_changed |= DragFloat(Label, &v[i], v_speed, v_min, v_max, display_format);
        SameLine(0, g.Style.ItemInnerSpacing.x);
        PopID();
        PopItemWidth();
        PopStyleColor();
